...
def plot_contagion_indicators():
    """
    Plot contagion indicators and overlay event-based and data-driven stress windows.
    """
    ...
    # Load event-based stress windows
    event_df = None
    ...
    # Load data-driven stress windows
    data_df = None
    ...
    # Helper to shade event windows
    def shade_event_windows(ax):
        if event_df is None or event_df.empty:
            return
        # shade each unique event period
        for event_name in event_df["Event_Name"].dropna().unique():
            sub = event_df[event_df["Event_Name"] == event_name]
            if "Window_Start" in sub.columns and "Window_End" in sub.columns:
                # one shading block per event name
                start = pd.to_datetime(sub["Window_Start"].iloc[0])
                end = pd.to_datetime(sub["Window_End"].iloc[0])
                ax.axvspan(start, end, color="gray", alpha=0.15, label="Event Stress" if event_name == event_df["Event_Name"].dropna().unique()[0] else None)

    # Helper to shade data-driven stress dates
    def shade_data_stress(ax):
        ...
        # Shade consecutive stress blocks
        ...
        for d in stress_dates[1:]:
            if (d - prev).days > 1:
                ax.axvspan(start, prev, color="red", alpha=0.08, label="Data-driven Stress")
                start = d
            prev = d
        ax.axvspan(start, prev, color="red", alpha=0.08, label="Data-driven Stress")
